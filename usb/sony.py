...
class SenserAuthenticationError(RuntimeError):
 pass


SenserAuthResult = namedtuple('SenserAuthResult', 'level keyId cameraId verified')


class SonySenserAuthDevice(SonyUsbDevice):
 """Senser v2 host handshake recovered from dump6 libsencore.so.

 The final authentication replies are acknowledgements, not proof that the
 camera accepted the signature. A subsequent service response is required.
 """
 # Confirmed in dump6 libInfraUsbCmn: setup patterns at 0x1eab2, registered
 # by CUsbGadgetSen::on_create. The libusb backend uses bmRequestType 0x43.
 ...
 def _sendAuthPacket(self, cmd, data=b''):
  if not 0 <= cmd <= 0xff or len(data) > 0x200:
   raise SenserAuthenticationError('Invalid authentication command or payload size')
  return self._exchangeAuthPacket(self.AuthPacket.pack(cmd=~cmd & 0xffff, salt=0, data=data))

 def _exchangeAuthPacket(self, outgoing):
  if len(outgoing) != self.AuthPacket.size:
   raise SenserAuthenticationError('Authentication wire frame must contain exactly 516 bytes')
  # Captured frames retain their original salted header and complete payload.
  self.driver.write(outgoing)
  packet = b''
  while len(packet) < self.AuthPacket.size:
   part = self.driver.read(self.AuthPacket.size - len(packet), timeout=5000)
   if not part or len(part) > self.AuthPacket.size - len(packet):
    raise SenserAuthenticationError('Incomplete or oversized authentication response')
   packet += part
  response = self.AuthPacket.unpack(packet)
  # CmdDecode @ 0x17560 keeps only the low eight bits after subtraction.
  ret = ((~response.cmd) - response.salt) & 0xff
  return ret, response.data

 @classmethod
 def _loadCapturedAuthFrame(cls, config, command):
  if not isinstance(config, dict) or not config.get('file'):
   raise SenserAuthenticationError('Configure the captured authentication frame file')
  try:
   path = os.path.expanduser(os.fspath(config['file']))
   if not os.path.isabs(path):
    path = os.path.join(os.path.dirname(__file__), path)
   with open(path, 'rb') as frameFile:
    raw = frameFile.read()
  except (OSError, TypeError, ValueError) as error:
   raise SenserAuthenticationError('Cannot read captured authentication frame') from error

  explicitRange = 'offset' in config or 'size' in config
  if explicitRange and not all(name in config for name in ('offset', 'size', 'sha256')):
   raise SenserAuthenticationError('An explicit capture range requires offset, size and source sha256')
  if 'sha256' in config and config['sha256'] != hashlib.sha256(raw).hexdigest():
   raise SenserAuthenticationError('Captured frame source SHA256 mismatch')
  if not explicitRange and len(raw) != cls.AuthPacket.size:
   raise SenserAuthenticationError('Capture is not 516 bytes; configure an explicit range and source sha256')
  offset, size = config.get('offset', 0), config.get('size', cls.AuthPacket.size)
  if (type(offset) is not int or type(size) is not int or offset < 0
      or size != cls.AuthPacket.size or offset + size > len(raw)):
   raise SenserAuthenticationError('Invalid captured frame range or size')
  packet = raw[offset:offset + size]
  parsed = cls.AuthPacket.unpack(packet)
  actualCommand = ((~parsed.cmd) - parsed.salt) & 0xff
  if actualCommand != command:
   raise SenserAuthenticationError('Captured frame command 0x%x does not match 0x%x' % (actualCommand, command))
  if offset != 0 or len(raw) != size:
   logging.getLogger(__name__).warning(
    'Capture %s: using bytes [%d:%d] of %d; excluded bytes have unknown framing',
    os.path.basename(path), offset, offset + size, len(raw))
  return packet

 @classmethod
 def _loadCaptureFrames(cls, config):
  if not isinstance(config, dict):
   raise SenserAuthenticationError('Configure STEP and JUMP capture frames')
  step = cls._loadCapturedAuthFrame(config.get('step'), 0x0a)
  jump = cls._loadCapturedAuthFrame(config.get('jump'), 0x0c)
  payload = step[4:]
  nonceOffset = payload[0]
  if nonceOffset <= 4 or nonceOffset + 32 > len(payload):
   raise SenserAuthenticationError('Invalid captured STEP nonce offset')
  # This replay mode supports the supplied single 512-byte JUMP payload.
  if parse32le(payload[2:6]) != len(jump) - 4:
   raise SenserAuthenticationError('Captured STEP signature length must match the 512-byte JUMP payload')
  return step, jump

 @staticmethod
 def _loadAuthKey(level, privateKeyFile, keyId, passphrase, expectedPublicKey=None):
  if level not in ('A', 'B', 'C'):
   raise SenserAuthenticationError('Authentication level must be A, B or C')
  if type(keyId) is not int or not 0 <= keyId <= 0x7fffffff:
   raise SenserAuthenticationError('Configure the numeric key_id from key_%s_public.json' % level)
  if not privateKeyFile:
   raise SenserAuthenticationError('Configure the RSA private_key_file for level %s; the firmware public key is insufficient' % level)
  try:
   with open(os.path.expanduser(privateKeyFile), 'rb') as keyFile:
    key = RSA.import_key(keyFile.read(), passphrase=passphrase)
  except (OSError, ValueError, TypeError, IndexError) as error:
   raise SenserAuthenticationError('Cannot load the configured RSA private key') from error
  if not key.has_private():
   raise SenserAuthenticationError('An RSA private key is required, not a public key')
  if expectedPublicKey is not None:
   try:
    publicKey = RSA.import_key(expectedPublicKey)
   except (ValueError, TypeError, IndexError) as error:
    raise SenserAuthenticationError('Cannot parse the configured camera public key') from error
   if publicKey.has_private():
    raise SenserAuthenticationError('Camera public_key_pem must contain only a public key')
   if (key.n, key.e) != (publicKey.n, publicKey.e):
    raise SenserAuthenticationError('RSA private key does not match the recovered level %s public key' % level)
  return key

 @staticmethod
 def _signAuthDigest(key, digest):
  if len(digest) != 32:
   raise SenserAuthenticationError('The authentication digest must contain 32 bytes')
  # 0x18100 uses verify_recover with PKCS#1 v1.5 padding and no signature_md.
  # The block contains the raw digest, WITHOUT an ASN.1 DigestInfo wrapper.
  paddingSize = key.size_in_bytes() - len(digest) - 3
  if paddingSize < 8:
   raise SenserAuthenticationError('RSA modulus is too small for the authentication digest')
  encoded = b'\x00\x01' + b'\xff' * paddingSize + b'\x00' + digest
  value = int.from_bytes(encoded, 'big')
  # PyCryptodome's own PKCS#1 signer uses this blinded private operation.
  signature = key._decrypt_to_bytes(value)
  if pow(int.from_bytes(signature, 'big'), key.e, key.n) != value:
   raise SenserAuthenticationError('RSA signature self-check failed')
  return signature

 @staticmethod
 def _checkAuthCount(data, expected):
  # 0x174c8 stores a LE32 count at a random nonzero payload-relative offset.
  offset = data[0]
  if offset == 0 or offset + 4 > len(data):
   raise SenserAuthenticationError('Invalid authentication acknowledgement offset')
  count = parse32le(data[offset:offset + 4])
  if count != expected:
   raise SenserAuthenticationError('Authentication byte count mismatch: %d != %d' % (count, expected))

 def _sendAuthSignature(self, signature):
  for offset in range(0, len(signature), 0x200):
   chunk = signature[offset:offset + 0x200]
   ret, data = self._sendAuthPacket(0x0c, chunk)
   count = offset + len(chunk)
   expected = 4 if count == len(signature) else 0x0b
   if ret != expected:
    raise SenserAuthenticationError('Unexpected JUMP reply: 0x%x' % ret)
   if ret == 0x0b:
    self._checkAuthCount(data, count)

 def authenticate(self, level=None, privateKeyFile=None, keyId=None, passphrase=None,
                  mode=None, captureFrames=None):
  """Exchange HOP/STEP/JUMP/KICK with RSA signing or captured STEP/JUMP frames.

  Capture mode never loads a private key or generates a new signature. Its
  recorded signature may fail against a fresh camera challenge. Both modes
  return verified=False: final ACKs alone do not prove signature acceptance.
  """
  level = constants.senserAuthLevel if level is None else level
  if level not in ('A', 'B', 'C'):
   raise SenserAuthenticationError('Authentication level must be A, B or C')
  profile = constants.senserAuthKeys.get(level, {})
  keyId = profile.get('key_id') if keyId is None else keyId
  if type(keyId) is not int or not 0 <= keyId <= 0x7fffffff:
   raise SenserAuthenticationError('Configure the numeric key_id from key_%s_public.json' % level)
  mode = constants.senserAuthMode if mode is None else mode
  if mode == 'capture':
   if privateKeyFile is not None or passphrase is not None:
    raise SenserAuthenticationError('Capture mode does not use private keys; choose mode="rsa" to sign')
   captureFrames = constants.senserAuthCaptureFrames if captureFrames is None else captureFrames
   stepFrame, jumpFrame = self._loadCaptureFrames(captureFrames)
   logging.getLogger(__name__).warning(
    'Replaying captured STEP/JUMP without a private key; ACKs do not verify authentication')
  elif mode == 'rsa':
   if captureFrames is not None:
    raise SenserAuthenticationError('Capture frames require mode="capture"')
   privateKeyFile = profile.get('private_key_file') if privateKeyFile is None else privateKeyFile
   key = self._loadAuthKey(level, privateKeyFile, keyId, passphrase, profile.get('public_key_pem'))
  else:
   raise SenserAuthenticationError('Authentication mode must be "capture" or "rsa"')

  # xxhop2 @ 0x17964 selects the JSON key by the ASCII level in packet[4].
  ret, data = self._sendAuthPacket(1, level.encode('ascii'))
  if ret != 9 or data[0] != 2:
   raise SenserAuthenticationError('Camera did not return a Senser v2 HOP response')
  cameraId = data[1:17].split(b'\0', 1)[0]
  cameraKeyId = parse32le(data[0x11:0x15])
  if cameraKeyId != keyId:
   raise SenserAuthenticationError('Camera key ID %d does not match configured key ID %d' % (cameraKeyId, keyId))
  if mode == 'capture':
   # Preserve the supplied 02 STEP bytes, including nonce offset and salt.
   ret, data = self._exchangeAuthPacket(stepFrame)
  else:
   cameraNonce = data[0x15:0x35]
   hostNonce = os.urandom(32)
   digest = hashlib.sha256(hostNonce + cameraNonce).digest()
   signature = self._signAuthDigest(key, digest)
   # xxstep2 reads length at packet[6:10] and nonce at packet[4+packet[4]].
   ret, data = self._sendAuthPacket(0x0a, b'\x06\x00' + dump32le(len(signature)) + hostNonce)
  if ret != 0x0b:
   raise SenserAuthenticationError('Unexpected STEP reply: 0x%x' % ret)
  self._checkAuthCount(data, 0)
  if mode == 'capture':
   ret, data = self._exchangeAuthPacket(jumpFrame)
   if ret != 4:
    raise SenserAuthenticationError('Unexpected JUMP reply: 0x%x' % ret)
  else:
   self._sendAuthSignature(signature)

  # xxkick2/xxkick: marker at packet[4], optional metadata at packet[10],
  # LE16 metadata length at packet[0x202]. This client sends no metadata.
  ret, data = self._sendAuthPacket(7, b'\xff' + b'\0' * 0x1ff)
  if ret != 6:
   raise SenserAuthenticationError('Unexpected KICK reply: 0x%x' % ret)
  res = parse8(data[:1])
  if res != 1:
   raise SenserAuthenticationError('Senser KICK error %d' % res)
  return SenserAuthResult(level, cameraKeyId, cameraId, False)
